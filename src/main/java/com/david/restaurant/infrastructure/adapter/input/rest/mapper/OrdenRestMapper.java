...
import com.david.restaurant.infrastructure.adapter.input.rest.dto.ordenDto.OrdenCocinaIngredienteResponse;
...
public class OrdenRestMapper {
    ...
    public Orden toUpdateDomain(OrdenUpdateRequest request) {
        if (request == null) return null;
        Orden orden = new Orden();
        if (request.tipoOrden() != null) {
            orden.setTipoOrden(TipoOrden.valueOf(request.tipoOrden()));
            ...
            ? orden.getItems().stream().map(item -> new OrdenCocinaItemResponse(
                ...
                item.getIngredientes() != null
                    ? item.getIngredientes().stream()
                        .filter(ing -> ing.getIngrediente() != null)
                        .map(ing -> new OrdenCocinaIngredienteResponse(
