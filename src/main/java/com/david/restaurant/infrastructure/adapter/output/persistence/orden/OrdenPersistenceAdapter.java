...
public class OrdenPersistenceAdapter implements OrdenRepositoryPort {
    ...
    public Pagina<Orden> findHistorialPaginado(int page, int size) {
        List<EstadoOrden> estados = List.of(EstadoOrden.CANCELADA, EstadoOrden.PAGADA);
        Slice<OrdenEntity> slice = ordenRepository.findHistorial(estados, EstadoOrden.ENTREGADA, PageRequest.of(page, size));
        List<Orden> contenido = slice.getContent().stream()
            .map(ordenMapper::toDomain)
            .collect(Collectors.toList());
        return new Pagina<>(contenido, slice.hasNext());
    }
